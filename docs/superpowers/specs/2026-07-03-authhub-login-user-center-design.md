...
- 邮箱
- 密码
- 邀请码，可选；如果来自 URL `?invite=<code>`，自动填入且不能修改
- 注册按钮
- 切换到登录
...
- 页面打开时如存在 `?invite=<code>` 且当前用户未登录，自动打开登录/注册弹窗，并默认切到注册态。
- URL 邀请码自动填入注册表单的邀请码字段，并锁定为只读，用户不能修改。
- 已登录用户访问带 `?invite=<code>` 的页面时，不自动弹出注册弹窗，也不自动使用该邀请码，避免误把邀请码绑定到已有账号。
...
- 未登录访问带 `?invite=<code>` 的页面会自动打开注册态弹窗，并锁定邀请码字段。
- 登录且会员有效时可选择 Pro 模型。
